...
function initial(info) {
    ...
    $("body").find("img.image").click(function() {
        ...
        if (state == 4) {
            window.webkit.messageHandlers.clickImage.postMessage({
                index: index,
                url: url
            });
        }
...
function configFontSize(size, className) {
    ...
    $(o).css({
        ...
        "line-height": parseFloat(size) * 1.5 + "px"
    });
}
/**
 获取图片url
...
function configImgState(state, index, className) {
    ...
    switch (state) {
        case 0:
            oImg.attr({
                "src": "../contentimage/load_image_initload.png",
                "alt": "米尔军事",
                "data-state": 0
            });
            break;
        case 1:
            oImg.attr({
                "src": "../contentimage/load_image_loading.png",
                "alt": "加载中",
                "data-state": 1
            }); //添加加载图片的路径
            break;
        case 2:
            oImg.attr({
                "src": "../contentimage/load_image_loadfail.png",
                "alt": "加载失败",
                "data-state": 2
            });
            break;
        case 3:
            oImg.attr({
                "src": "../contentimage/load_image_clickload.png",
                "alt": "点击加载",
                "data-state": 3
            });
            break;
...
function alertContent(className) {
    alert(className);
}
